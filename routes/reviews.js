...
// Reviews New Seeker
router.get("/user/:id/reviews/new/seeker", middleware.isLoggedIn, middleware.checkReviewExistence, function (req, res) {
    // middleware.checkReviewExistence checks if a user already reviewed the user, only one review per user is allowed
    User.findById(req.params.id, function (err, user) {
        if (err) {
            req.flash("error", err.message);
        ...
        res.render("reviews/newSeeker.ejs", {user: user});
    });
});
...
// Reviews Create Seeker
router.post("/user/:id/reviews/seeker", middleware.isLoggedIn, middleware.checkReviewExistence, function (req, res) {
    //lookup user using ID
    User.findById(req.params.id).populate("reviews").exec(function (err, user) {
        if (err) {
            req.flash("error", err.message);
...
// Reviews Edit Seeker
router.get("/user/:id/reviews/:review_id/edit/seeker", middleware.checkReviewOwnership, function (req, res) {
    Review.findById(req.params.review_id, function (err, foundReview) {
        if (err) {
            req.flash("error", err.message);
            return res.redirect("back");
        ...
        res.render("reviews/editSeeker.ejs", {user_id: req.params.id, review: foundReview});
    });
});
...
// Reviews Update Seeker
router.put("/user/:id/reviews/:review_id/seeker", middleware.checkReviewOwnership, function (req, res) {
    Review.findByIdAndUpdate(req.params.review_id, req.body.review, {new: true}, function (err, updatedReview) {
        if (err) {
            req.flash("error", err.message);
            return res.redirect("back");
...
// Reviews Delete Seeker
router.delete("/user/:id/reviews/:review_id/seeker", middleware.checkReviewOwnership, function (req, res) {
    Review.findByIdAndRemove(req.params.review_id, function (err) {
        if (err) {
            req.flash("error", err.message);
            return res.redirect("back");
